...
o \gls{mta}, o \textit{plugin AccTrace} e os outros recursos relacionados neste trabalho. Os atores elencados são
fictícios, tendo os autores deste trabalho como especialista em acessibilidade.
O objetivo não é aplicar a prova de conceito no \gls{mta}, por isso nem todos
...
No Subprocesso 1 (Elicitação dos Requisitos do Sistema), observando a Tarefa
1.1 (Identificar os Requisitos de Acessibilidade do Sistema) do \gls{mta}
\citep{maia:10}, o especialista em acessibilidade identificou que:
...
\begin{itemize}
  ...
  acessado independente de dispositivo ou tecnologia;
...
Nenhum requisito de acessibilidade foi identificado nesta etapa; até
...
Partindo para o Subprocesso 2 (Análise de Requisitos do Sistema), na Tarefa
2.1 (Especificar os Requisitos de Acessibilidade do Sistema) \citep{maia:10}, o
...
O especialista em acessibilidade constatou que 2 requisitos não funcionais de
...
feita na Tarefa 2.2 (Avaliar os Requisitos de Acessibilidade do Sistema)
\citep{maia:10} e registrada no documento de requisitos de acessibilidade do sistema
revisado.
...
Requisitos de Acessibilidade aos Elementos do Sistema) e 3.2 (Avaliar o Projeto
Arquitetural do Sistema com Relação aos Requisitos de Acessibilidade) \citep{maia:10} tratam da decomposição do
...
anteriormente, é definido que, para cada um dos requisitos não funcionais
...
O requisito não funcional ``os elementos do sistema devem ser: perceptíveis,
operáveis, compreensíveis e robustos'' deve permear todos os componentes do projeto, inclusive os já descritos anteriormente.
...
(RNF), inclusive os de acessibilidade (RNFA), podem ser vistos abaixo:

\begin{itemize}
  \item RF1: o sistema deve permitir que, dado uma chave de busca (uma ou mais
  palavras), o resultado da busca seja retornado;
  \item RNF1: o sistema deve retornar o resultado da busca em menos de 2
  segundos, para não prejudicar a percepção do usuário;  
  \item RNF2: o sistema deve permitir que o usuário altere a linguagem padrão da
  ferramenta;  
  \item RNF3: o sistema deve permitir que o usuário altere o constraste da
  página;  
   \item RNF4: o sistema deve permitir que o usuário altere o tamanho da fonte
   da página; 
   \item RNF5: o sistema deve permitir que o usuário informe qual é o tipo de
  limitação/deficiência possui; 
   \item RNF6: o sistema deve fornecer atalhos via teclado, para alterar o
   idioma e a informação sobre a limitação/deficiência, sempre voltando o foco para a caixa
  de texto principal; 
   \item RNFA1: o sistema deve seguir a recomendação de
  acessibilidade \gls{wcag} 2.0, nível AAA; 
  ...
  conteúdo seja suficiente;
  \item RNFA4: o sistema deve entregar um conteúdo que não cause convulsão;
  \item RNFA5: o sistema deve permitir que o usuário navegue facilmente e
  encontre o conteúdo desejado.  
  \item RNFA6: o sistema deve utilizar a linguagem especificada como critério de
  seleção no mecanismo de busca;
  \item RNFA7: o sistema deve utilizar o tipo de limitação/deficiência como
  critério de seleção no mecanismo de busca;
   \item RNFA8: o sistema deve permitir que todas as funcionalidades sejam
   acessíveis via teclado;
  \item RNFA9: o sistema deve permitir que, ao ser especificado o tipo de
  limitação/deficiência, a utilização de métrica de acessibilidade sejam
  utilizadas para classificar as páginas.    
\end{itemize}
...
categoria de Requisitos Não Funcionais possui um Subcategoria (Requisitos Não
Funcionais de Acessibilidade). A Figura \ref{fig:facomsearchrequirementplugin}
mostra o cadastro dos requisitos na ferramenta.

\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./images/facomsearchrequirementplugin.png}
...
\ref{fig:facomsearchusescase}, assim como o diagrama de classes  de análise pode
ser visto na Figura \ref{fig:facomsearchclassdiagram}. Como a implementação final deste
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./images/facomsearchusescase.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{./images/facomsearchclassdiagram.png}
\caption{Diagrama de classes de análise do buscador}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./images/referenciascriadas.png}
...
momento. A Figura \ref{fig:facomsearchclassdiagram_expanded} mostra o diagrama
de classes de projeto do busca (expansão do diagrama de classes de análise).

\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./images/facomsearchclassdiagram_expanded.png}
\caption{Diagrama de classes de projeto do buscador}
\label{fig:facomsearchclassdiagram_expanded}
\end{figure}

Na Subtarefa Estabelecer a disposição dos elementos na
...
``Definir atalhos de navegação'', mas é possível sugerir genericamente um que
...
de acessibilidade no \textit{plugin AccTrace}. É importante notar que não são tratados apenas
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./images/tecnicasassociadas.png}
\caption{Associação das técnicas de acessibilidade para um modelo e requisito
selecionado.}
\label{fig:tecnicasassociadas}
\end{figure}

Neste ponto é possível gerar a matriz de rastreabilidade. As Figuras \ref{fig:reqtec} e
\ref{fig:modeltec} mostram uma parte dos dados gerados, para a associação de
requisitos e técnicas, bem como modelos e técnicas.

\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./images/reqtec.png}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./images/modeltec.png}
...
executáveis que apropriadamente refletem o projeto do software. A Tarefa
6.1 (Especificar Técnicas para Implementação da Acessibilidade da Interface e do
Código), segundo \citet{maia:10}, sugere que técnicas específicas para atender aos requisitos de acessibilidade do sistema deverão ser pesquisadas tais como o
...
Acessibilidade) refere-se à construção propriamente dita da solução proposta \citep{maia:10}. O
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./images/generatedcode.png}
\caption{Visualização do código gerado, com os comentários \textit{AccTrace}
personalizados}
\label{fig:generatedcode}
\end{figure}

Os desenvolvedores do \textit{software} podem partir destes códigos \textit{stub}
e continuar a desenvolvimento da ferramenta, sendo beneficiados pelos comentários
...
Unidade de Software) \citep{maia:10}, que estão fora do escopo desta dissertação. É importante
...
complexa, inclusive para a atualização do modelo \gls{wcag} 2.0, se houver diferenças na estrutura já utilizada na ontologia usada neste trabalho.
Caso a estrutura entre os elementos da ontologia não seja alterada, não é necessário fazer alterações no \textit{plugin AccTrace}.
...
propostos foram atingidos. A seguir, apresenta-se como os objetivos propostos para abordagem desenvolvida neste trabalho foram atingidos:

\begin{itemize}
  \item \textbf{desenvolvimento de uma ferramenta que seja orientada ao desenvolvedor (a apresentação dos resultados
  ...
  \item \textbf{desenvolvimento de uma ferramenta que seja integrada ao ambiente de desenvolvimento do
  ...
  \item \textbf{desenvolvimento de uma ferramenta que apresente informações objetivas, e no momento em que o
  ...
  \item \textbf{desenvolvimento de uma ferramenta que tenha relação direta entre os requisitos e casos de uso com a
  ...
  recuperados na etapa de codificação;
  \item \textbf{desenvolvimento de uma ferramenta que permita que seja feita o rastreamento dos requisitos de
  ...
  \item \textbf{desenvolvimento de uma ferramenta que permita que o desenvolvedor consiga verificar, em nível de
