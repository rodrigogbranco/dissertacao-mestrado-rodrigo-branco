...
\begin{itemize}
  \item \gls{mta} - Processo de Desenvolvimento de Software acessível
  \item Eclipse - \gls{ide}
  \item \gls{rmf} - Conjunto de ferramentas \gls{case} de Gerenciamento de
  Requisitos para Eclipse
  \item \gls{emf} - Conjunto de ferramentas \gls{case} de Modelagem \gls{uml}
  \item Ferramenta \gls{case} de rastreabilidade de requisitos de
  acessibilidade\footnote{Ferramenta de apoio proposta por este trabalho}
  \item Java - Linguagem para desenvolvimento de \textit{plugins} para o Eclipse
  \item Ontologias para implementação das diretrizes do \gls{wcag} 2.0.
\end{itemize}

O \textit{Eclipse} é uma plataforma madura foi escolhido como \gls{ide} por
vários motivos.
O principal deles é que as duas ferramentas \gls{case} usadas (\gls{rmf} e \gls{emf}) são
\textit{plugins} para a plataforma. O projeto utiliza a licença \textit{Eclipse Public License v
1.0}\footnote{http://www.eclipse.org/legal/epl-v10.html}, o que permite
a verificação do código e possíveis intervenções. As ferramentas \gls{case}
gerenciam seus artefatos usando arquivos \gls{xml}, formato legível por máquina
escolhido para este trabalho. O desenvolvimento da ferramenta \gls{case}
proposta é feita no próprio ambiente (desenvolvimento de \textit{plugin}),
facilitando bastante este trabalho\footnote{Um tutorial de desenvolvimento de plugins para o Eclipse
pode ser encontrado em http://www.ibm.com/developerworks/br/library/os-ecplug/}.
Por fim, o desenvolvimento do respectivo \textit{plugin} é feito na linguagem
Java, a linguagem nativa do ambiente. O trabalho foi desenvolvido usando a
versão \textit{VIndigo Service Release 2} do \textit{Eclipse}.
