...
A ontologia é descrita no formato \gls{owl}, padrão definido pela \gls{w3c}
em 2004 \citep{owl:13}, está na versão 1.0. O formato \gls{owl} já possui uma versão candidata à 2.0, lançada em 2012 \citep{owl2draft:13}, que até o momento de escrita deste trabalho não foi oficializada como versão oficial. As versões aqui descritas em nada influem na versão do \gls{wcag} mapeado na ontologia utilizada, que é a versão 2.0 do mesmo.
...
de código, pois o código fonte deste gerador estava disponível e, e o gerador de código usa
...
A Figura \ref{fig:figuramagica} mostra a macroabordagem escolhida que, partindo da Engenharia de Requisitos e chegando na construção do \textit{software},
...
baixados em
https://github.com/ObeoNetwork/InformationSystem/tree/master/designs/requirement}, seja possível utilizar as classes java resultantes e, principalmente, que o arquivo de saída da ferramenta \textit{Requirement Designer} possa ser lido e interpretado de forma simplificada como os objetos
...
matriz de todos os Requisitos associados aos respectivos modelos, mesmo que
exista algum Requisito que não esteja associado a algum modelo. Essa situação
está prevista justamente para identificar problemas na associação que devam ser
corrigidas. Para as planilhas Requisitos x Técnicas e Modelos x Técnicas,
